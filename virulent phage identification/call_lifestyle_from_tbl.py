...
# 原始 phage genome fasta 根目录（用于算 genome length）
...
def read_hmm_ids(tbl_path):
    """
    读取 hmmsearch --tblout 输出。
    你已验证：第3列 (query name) = HMM ID（phrog_/VOG/...）
    """
    ids=set()
...
def lifestyle_call(has_int, has_terl, has_lysis):
    # 当前宽松版本：有 integrase => temperate；无 integrase => 先当 virulent
    if has_int:
        return "temperate"
    # dsDNA tailed 的结构性前提（terL）+ lysis 支持 => strong virulent
    if has_terl and has_lysis:
        return "strong_virulent"
    if has_terl:
        return "virulent"
    # 没有 terL：可能不是 dsDNA tailed / 或者是不完整 contig / annotation缺失
    return "uncertain"
...
def find_fasta(data_root, src, pid):
    ...
    # 2) 兜底：pid.* 里找 fasta/fa/fna 或其 gz（应对极少数命名差异）
    for f in glob.glob(str(base / f"{pid}.*")):
        if f.endswith(FA_EXTS) or any(f.endswith(e + ".gz") for e in FA_EXTS):
            try:
...
def fasta_length_bp(fasta_path):
    """
    Sum of all sequence lines (ignore headers).
    If multi-contig, returns total bp across contigs.
    """
    total = 0
...
def main():
    ...
    with open(LIST_TSV) as f:
        ...
        for row in r:
            if not row or len(row) < 2:
                continue
            src, pid = row[0].strip(), row[1].strip()
    ...
    for src, pid in master:
        ...
        if not tbl.exists() or tbl.stat().st_size == 0:
            miss_tbl += 1
            rows.append([src, pid, genome_len, 0, 0, 0, 0, 0, 0, 0, "missing_tbl"])
            continue
        ...
        call = lifestyle_call(has_int, has_terl, has_lys)

        rows.append([
            src, pid, genome_len,
            n_total,
            len(hit_int), len(hit_terl), len(hit_lys),
            has_int, has_terl, has_lys,
            call
        ])
    ...
    with open(OUT_TSV, "w", newline="") as fo:
        ...
        w.writerow([
            "Phage_source","Phage_ID","Genome_length_bp",
            "n_unique_hmm_hits",
            "n_integrase_hits","n_terL_hits","n_lysis_hits",
            "Has_integrase","Has_terL","Has_lysis",
            "Lifestyle_call"
        ])
        w.writerows(rows)

    print("[DONE]", OUT_TSV)
    print("[INFO] master:", len(master), " missing_tbl:", miss_tbl, " missing_fasta:", miss_fa)
